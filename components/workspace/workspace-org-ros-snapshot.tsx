...
export function WorkspaceOrgRosCtaCard({
  ...
  return (
    <Link
      href={href}
      className="group flex flex-col gap-3 rounded-3xl border border-border/50 bg-card/80 p-4 shadow-sm ring-1 ring-black/[0.04] transition-all hover:-translate-y-0.5 hover:shadow-md dark:ring-white/[0.06]"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="font-heading font-semibold leading-snug">Organisasjon</p>
          <p className="text-muted-foreground mt-1 text-sm leading-relaxed">
            {orgUnits.length === 0
              ? "Opprett enheter og knytt prosesser til riktig sted."
              : `${orgUnits.length} enhet${orgUnits.length === 1 ? "" : "er"} i kartet med prosess- og ROS-oversikt.`}
          </p>
        </div>
        <ChevronRight
