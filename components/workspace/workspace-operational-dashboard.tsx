    ...
    <Link href={href} className="group block h-full">
      ...
      >
        ...
        <CardContent>
          ...
          <CardDescription className="mt-1.5 text-xs leading-snug">
            {hint}
          </CardDescription>
        </CardContent>
      </Card>
    ...
    return (
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="bg-muted/40 h-28 animate-pulse rounded-xl border border-border/50"
          />
        ))}
      </div>
    );
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="space-y-3">
          ...
          {recentlyUpdated.length === 0 ? (
            <div className="rounded-xl border border-dashed bg-muted/15 px-4 py-6 text-center">
...
function AssessmentDashRow({
  ...
  return (
    <Link
      ...
      className="hover:border-primary/30 flex flex-col gap-1 rounded-xl border border-border/60 bg-card p-3 text-left shadow-sm transition-all hover:bg-muted/30"
    >
